...
class TestRailProject(object):
    ...
    def add_plan_entry(self, name, plan_id, suite_id, config_ids, runs):
    ...
    def update_run(self, run_id, milestone_id=None, description=None,
                   config_ids=None, include_all=None, case_ids=None,
                   name=None):
        update_run_uri = 'update_run/{}'.format(run_id)
        update_run = {}
        if name:
            update_run['name'] = name
        if milestone_id:
    ...
    def get_default_priority(self):
        priorities = self.get_prperties()
        return (prio for prio in priorities if prio['is_default']).next()
