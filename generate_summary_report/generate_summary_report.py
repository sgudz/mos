...
HOSTNAME = "http://mos-scale.vm.mirantis.net"
EXECUTION_RESULTS = "/var/www/test_results"
OUTPUT_HTML_REPORT = "/var/www/test_results/index.html"
...
def _parse_filename(filename):
    ...
    if not match:
        logging.error("Invalid name in '%s', skipped." % filename)
        return
    else:
        ...
        data["jenkins_job"] = {
            "url": "/".join(["%s:8080" % HOSTNAME, "job", job_name, job_id]),
        ...
        data["results_url"] = filename.replace(
            EXECUTION_RESULTS, os.path.join(HOSTNAME, "test_results"))
    ...
    for root in files:
        ...
        if filter(lambda filename: filename.endswith(".xml") or
            ...
            junit_file = os.path.join(root,
                                      "%s-report.xml" % execution["executor"])
